...
export default function Like({likesData, id}:any) {
    ...
    useEffect(() => {
        if(Object.keys(profileData).length > 0){
            const userLikesData = JSON.parse(profileData.likes_problem_post)
    ...
    const handleLikeButton = () => {
        if(profileData.id){
            ...
            FetchMainApi({url:`/problem-post/like/${id}`, method:"put", header:header, data:reqData})
                .then((res) => {
                    dispatch(updateProfileProperty("likes_problem_post", res.data.ids));
                })
                .catch((err) => {
                    // if(!hasLiked){
                    //     setTotalLiked(totalLiked - 1)
